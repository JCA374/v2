...
logging.basicConfig(level=logging.INFO, 
                    ...
                    handlers=[
                        ...
                        logging.FileHandler("swedish_stocks_alphavantage_load.log")
                    ])
logger = logging.getLogger(__name__)

# List of Swedish stocks to load using Alpha Vantage format
# For Swedish stocks, Alpha Vantage uses format like ERIC.STO instead of ERIC-B.ST
SWEDISH_STOCKS = [
    {"yahoo": "ERIC-B.ST", "alphavantage": "ERIC-B.STO", "name": "Ericsson"},
    {"yahoo": "VOLV-B.ST", "alphavantage": "VOLV-B.STO", "name": "Volvo"},
    {"yahoo": "SAND.ST", "alphavantage": "SAND.STO", "name": "Sandvik"},
    {"yahoo": "SEB-A.ST", "alphavantage": "SEB-A.STO", "name": "SEB"},
    {"yahoo": "ASSA-B.ST", "alphavantage": "ASSA-B.STO", "name": "ASSA ABLOY"},
    {"yahoo": "ATCO-A.ST", "alphavantage": "ATCO-A.STO", "name": "Atlas Copco"},
    {"yahoo": "SHB-A.ST", "alphavantage": "SHB-A.STO", "name": "Handelsbanken"},
    {"yahoo": "INVE-B.ST", "alphavantage": "INVE-B.STO", "name": "Investor"},
    {"yahoo": "EVO.ST", "alphavantage": "EVO.STO", "name": "Evolution Gaming"},
    {"yahoo": "SWED-A.ST", "alphavantage": "SWED-A.STO", "name": "Swedbank"}
]

# Alternatively, let's add some US stocks that we know work well with Alpha Vantage
US_STOCKS = [
    {"yahoo": "AAPL", "alphavantage": "AAPL", "name": "Apple"},
    {"yahoo": "MSFT", "alphavantage": "MSFT", "name": "Microsoft"},
    {"yahoo": "GOOGL", "alphavantage": "GOOGL", "name": "Google"},
    {"yahoo": "AMZN", "alphavantage": "AMZN", "name": "Amazon"},
    {"yahoo": "NVDA", "alphavantage": "NVDA", "name": "NVIDIA"}
]
...
def fetch_company_overview(api_key, ticker):
    ...
    try:
        ...
        if "Symbol" in data and data["Symbol"]:
            ...
            logger.error(f"Failed to get company overview for {ticker}: {data}")
            return None
...
def fetch_daily_prices(api_key, ticker, output_size='full'):
    """
...
def load_stocks_from_alphavantage(stock_list, api_key, db):
    """
    Load stock data from Alpha Vantage API and store in Supabase.
    
    Args:
        stock_list: List of stock dictionary objects with yahoo and alphavantage tickers
        api_key: Alpha Vantage API key
    ...
    for stock in stock_list:
        ...
        alpha_ticker = stock['alphavantage']
        
        try:
            logger.info(f"Processing {yahoo_ticker} (Alpha Vantage: {alpha_ticker})")
            ...
            if call_count >= 5 and current_time - call_start_time < 65:
                ...
                logger.info(f"Waiting {wait_time:.1f} seconds to respect API rate limits...")
            ...
            if fundamental_data:
                ...
                if not fundamental_data.get('name') or fundamental_data['name'] == alpha_ticker:
                    fundamental_data['name'] = stock.get('name', yahoo_ticker)
                
                # Save fundamental data using Yahoo ticker for consistency
                success = db.save_fundamental_data(yahoo_ticker, fundamental_data, source="alphavantage")
                logger.info(f"Saved fundamental data for {yahoo_ticker}: {success}")
            else:
                ...
                db.save_fundamental_data(yahoo_ticker, backup_data, source="alphavantage")
            ...
            if daily_data is not None and not daily_data.empty:
                success = db.save_price_data(yahoo_ticker, daily_data, source="alphavantage")
                logger.info(f"Saved daily price data for {yahoo_ticker}: {success}")
            ...
            if weekly_data is not None and not weekly_data.empty:
                success = db.save_price_data(yahoo_ticker, weekly_data, source="alphavantage")
                logger.info(f"Saved weekly price data for {yahoo_ticker}: {success}")
            ...
            if monthly_data is not None and not monthly_data.empty:
                success = db.save_price_data(yahoo_ticker, monthly_data, source="alphavantage")
                logger.info(f"Saved monthly price data for {yahoo_ticker}: {success}")
...
def main():
    ...
    try:
        ...
        if choice == "1":
            ...
            load_stocks_from_alphavantage(US_STOCKS, alpha_vantage_api_key, db)
        elif choice == "2":
            logger.info("Loading Swedish stocks")
            load_stocks_from_alphavantage(SWEDISH_STOCKS, alpha_vantage_api_key, db)
        else:
            logger.info("Loading all stocks")
            all_stocks = US_STOCKS + SWEDISH_STOCKS
            load_stocks_from_alphavantage(all_stocks, alpha_vantage_api_key, db)
