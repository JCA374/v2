"""
Script to load data for US stocks into the Supabase database using Alpha Vantage API.
US stocks are more reliable with Alpha Vantage than Swedish stocks.
"""
import pandas as pd
import time
import logging
...
logging.basicConfig(level=logging.INFO, 
                    ...
                    handlers=[
                        ...
                        logging.FileHandler("us_stocks_alphavantage_load.log")
                    ])
logger = logging.getLogger(__name__)

# List of US stocks to load (these work reliably with Alpha Vantage)
US_STOCKS = [
    {"yahoo": "AAPL", "name": "Apple Inc"},
    {"yahoo": "MSFT", "name": "Microsoft Corporation"},
    {"yahoo": "GOOGL", "name": "Alphabet Inc"},
    ...
    {"yahoo": "PG", "name": "Procter & Gamble Co"}
]
...
def load_stocks_from_alphavantage(stock_list, api_key, db):
    ...
    for stock in stock_list:
        ...
        try:
            # For US stocks, Alpha Vantage and Yahoo use the same ticker format
            alpha_ticker = yahoo_ticker
            
            logger.info(f"Processing {yahoo_ticker}")
            ...
            daily_data = fetch_daily_prices(api_key, alpha_ticker, output_size="full")
            call_count += 1
...
def main():
    ...
    logger.info("Starting US stocks data load using Alpha Vantage API")
    ...
    try:
        ...
        # Load US stock data into Supabase
        load_stocks_from_alphavantage(US_STOCKS, alpha_vantage_api_key, db)
        
        logger.info("Completed loading US stocks data")
